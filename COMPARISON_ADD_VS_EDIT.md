...
**🔑 Customer Code Fix (New - 2026-02-24):**
- Customer Code **enable อัตโนมัติทันที** เมื่อเลือก "ลูกค้าเก่า"
- ไม่ต้องรอเลือก person type หรือติ๊ก checkbox ใดๆ
- ใช้สำหรับค้นหา/เลือกลูกค้า (autocomplete) เป็น Primary Field
- เอกสารเก่าผิด - ได้แก้ไขแล้วทั้งหมด

**🔐 Form Locking Strategy (2026-02-25):**
...
**Version:** 2.3 (Customer Code Fix + Form Locking Pattern)  
...
## � Customer Code Pattern (สำคัญ! - อัปเดต 2026-02-24)

### ⚠️ เอกสารเดิมผิด!

**เอกสารเก่าบอกว่า:** ❌
> Customer code จะ enable ได้เฉพาะเมื่อติ๊ก "เปลี่ยนเขตการขาย" หรือ "แก้ไขข้อมูลลูกค้า"

**ความจริง:** ✅
> Customer code **enable อัตโนมัติทันที** เมื่อเลือก "ลูกค้าเก่า" (OLD customer)

### พฤติกรรมที่ถูกต้อง

#### สำหรับลูกค้าเก่า (OLD Customer)

| เหตุการณ์ | Customer Code State | หมายเหตุ |
|-----------|-------------------|----------|
| เลือก "ลูกค้าเก่า" | ✅ **Enabled ทันที** | ไม่ต้องรอ person type หรือ checkbox |
| พิมพ์ค้นหา | ✅ Autocomplete ทำงาน | ระบบแสดงรายการลูกค้าที่ตรงกับ code |
| เลือกลูกค้า | ✅ Readonly (แต่เก็บค่า) | ไม่สามารถเปลี่ยนได้ (เป็น primary key) |
| ติ๊ก checkbox ต่างๆ | ⚠️ ยัง Readonly | Customer code ไม่เปลี่ยนแปลง |

#### Implementation (addth-main.js)

```javascript
// ✅ CORRECT: Enable customer code immediately after disabling all fields
function initializeOldCustomerHandlers() {
    disableAllFieldsForOldCustomer();
    
    // IMPORTANT: Enable customer code for search functionality
    // This is REQUIRED for autocomplete/customer selection
...
### เหตุผลที่ต้อง Enable ทันที

1. **Customer Code คือช่องค้นหา (Search Field)**
   - ใช้สำหรับ autocomplete เพื่อหาลูกค้าเดิม
   - ถ้า disabled → ไม่สามารถค้นหาได้เลย

2. **ไม่ได้ถูกควบคุมโดย Checkbox**
   - Checkbox ควบคุม**ฟิลด์อื่นๆ** ที่จะแก้ไขได้หลังเลือกลูกค้า
   - Customer code ต้อง enable **ก่อน** เพื่อให้เลือกลูกค้าได้

3. **เป็น Primary Key**
   - หลังเลือกลูกค้าแล้ว จะกลายเป็น readonly (ไม่ให้เปลี่ยน)
   - เพราะ customer code ใช้ระบุลูกค้า ห้ามเปลี่ยนระหว่างแก้ไข

### เปรียบเทียบ: เก่า vs ใหม่

| Aspect | เอกสารเก่า (ผิด) | ความจริง (ถูก) |
|--------|---------------|--------------|
| **Enable Time** | ❌ เมื่อติ๊ก checkbox | ✅ ทันทีเมื่อเลือก OLD |
| **Dependencies** | ❌ Person type หรือ checkbox | ✅ ไม่มี - Enable ทันที |
| **Purpose** | ❌ แก้ไขข้อมูล | ✅ ค้นหา/เลือกลูกค้า |
| **After Selection** | ? Unclear | ✅ Readonly (locked) |

### Code Changes (2026-02-24)

**แก้ไขแล้ว:**
- ✅ `addth-main.js` line 407-411: Enable customer code after `disableAllFieldsForOldCustomer()`
- ✅ `addth-main.js` line 429-431: ลบ logic ที่ enable customer code เมื่อเลือก person type (ไม่จำเป็นแล้ว)
- ✅ `ADDTH_DOCUMENTATION.md`: อัปเดตทุก section ที่กล่าวถึง customer code
- ✅ `COMPARISON_ADD_VS_EDIT.md`: เพิ่ม section อธิบาย customer code pattern

**ผลลัพธ์:**
- ✅ Customer code ใช้งานได้ทันทีสำหรับลูกค้าเก่า
- ✅ ไม่ต้องรอเลือก person type หรือติ๊ก checkbox
- ✅ Autocomplete/search ทำงานปกติ
